...
#     '''
#     select(kwargs.get('number'))ame['n'] = kwargs.get('name')
#
#
# # 삭제(번호로 삭제)
# def delete(number):
...
# def insert(*, number: int, title: str, content: str, file: str, read_count = 0):
#     post_info.append({'number': number, 'title': title, 'content': content, 'file': file})
#
# insert(number=6, title='테스트 제목6', content='테스트 내용6', file='/usr/post/file/img006')
# print(post_info)
#
# # 목록(최신순으로 조회)
# def select_all():
...
# # 삭제(번호로 삭제)
# def delete(number):
#     del post_info[post_info.index(number)]
