...
while True:
    ...
    # 삭제
        ...
        if name in name_list:
            index = name_list.index(name)
            # 상품 목록에서 해당 인덱스의 상품명을 삭제합니다.
            del name_list[index]
            # 가격 목록에서 해당 인덱스의 가격을 삭제합니다.
        ...
        # 가격으로 검색
            ...
            if len(result_index) != 0:
                for i in result_index:
                    # 해당 인덱스 번호에 해당되는 상품명과 가격을 출력합니다.
                    print(f'{name_list[i]}, {price_list[i]}')
                    continue

            else:
                # 가격 범위 내의 상품이 없는 경우 search_error_message를 출력합니다.
