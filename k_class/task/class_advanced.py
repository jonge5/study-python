...
class Member:
    def __init__(self, id, number, password, name):
        self.number = number
        self.id = id
    ...
    @classmethod
    def admin_membership(cls, **kwargs):
        result = 'aimin_'+ kwargs.get('id')
        return Member(result, kwargs.get('number'), kwargs.get('password'), kwargs.get('name'))

manager1 = Member.admin_membership(number= 1, id='fififififi', password='1234', name='조은종')
print(manager1.id)
